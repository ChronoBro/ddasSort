...
int main(int argc,char *argv[]){
  ...
  for(long long int iEntry=0;iEntry<dataChain.GetEntries();iEntry=lastEntry+1){
    ...
    ****************************************/
    ...
    RBDDASChannel testChannel(trigChan);
    testChannel.setEventPointer(curChannel, pDDASEvent);
    //I can set this before I pass RBDDASChannel to RBDDTrigger and then the unpacking should work correctly

...
void threadedEventLoop(diagnostics &counterList, histo* Histo, RBDDTriggeredEvent* implant, RBDDTriggeredEvent* decay, int iStart, int iEnd){

  for(long long int iEntry=iStart;iEntry<iEnd;iEntry=lastEntry+1){
...
int main(int argc,char *argv[]){
  ...
  int total = dataChain.GetEntries();
  int block1;
  int block2;
  int block3;
  int block4;
